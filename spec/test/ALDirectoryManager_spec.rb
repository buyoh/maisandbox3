...
RSpec.describe ALDirectoryManager do
  it 'create dirs' do
    ...
    expect(Dir.exist?(ALBase.work_directory + user1_box1_dir)).to eq true
    user1_box2 = dm.new_box(user1)
    user1_box2_dir = dm.get_boxdir(user1, user1_box2)
    expect(Dir.exist?(ALBase.work_directory + user1_box2_dir)).to eq true
    ...
    expect(Dir.exist?(ALBase.work_directory + user2_box1_dir)).to eq true
    user2_box2 = dm.new_box(user2)
    user2_box2_dir = dm.get_boxdir(user2, user2_box2)
    expect(Dir.exist?(ALBase.work_directory + user2_box2_dir)).to eq true

    expect(user1_box1_dir).not_to eq user1_box2_dir
    expect(user1_box1_dir).not_to eq user2_box1_dir

    dm.uninstall_user user1
    expect(Dir.exist?(ALBase.work_directory + user1_box1_dir)).to eq false
    expect(Dir.exist?(ALBase.work_directory + user2_box1_dir)).to eq true
